...
function initializeLoginForm() {
    ...
    inputs.forEach(input => {
        input.addEventListener('input', function() {
            ...
            loginData[fieldName] = this.value
            clearLoginError(fieldName)
        })

        input.addEventListener('blur', function() {
            validateLoginField(this.id, this.value)
        })
    })

    form.addEventListener('submit', function(event) {
        event.preventDefault()
        handleLoginSubmit(submitLoginBtn)
    })
}
...
function validateLoginField(fieldName, value) {
    ...
    if (!isValid) {
        ...
        loginFormErrors[fieldName] = errorMessage
    } else {
        clearLoginError(fieldName)
        delete loginFormErrors[fieldName]
    }
...
function clearLoginError(fieldName) {
    const input = document.getElementById(fieldName)
    const errorElement = document.getElementById(`${fieldName}Error`)
    if (input && errorElement) {
        input.classList.remove('error')
...
function validateLoginForm() {
    let isFormValid = true
    for (const fieldName in loginData) {
        if (!validateLoginField(fieldName, loginData[fieldName])) {
            isFormValid = false
        }
    }
    return isFormValid
...
function handleLoginSubmit(submitLoginBtn) {
    if (!validateLoginForm()) {
        console.log("❌ Formulário contém erros. Corrija os campos destacados.")
        return
    }

    // Simula o loading no botão
    submitLoginBtn.disabled = true
    const originalText = submitLoginBtn.textContent
    submitLoginBtn.textContent = 'Entrando...'

    setTimeout(() => {
        console.log("=".repeat(50))
        console.log("✅ LOGIN REALIZADO COM SUCESSO!")
        console.log("DADOS DO LOGIN:")
        console.log(`E-mail: ${loginData.emailLogin}`)
        console.log(`Senha: ${loginData.passwordLogin}`)
        console.log("Data/Hora: ", new Date().toLocaleString("pt-BR"))
        console.log("=".repeat(50))

        alert("Login realizado com sucesso!\n\nVerifique o console do navegador (F12) para os dados enviados.")

        // Reset do formulário
        const form = document.getElementById('loginForm')
        form.reset()
        loginData = {emailLogin: '', passwordLogin: ''}
        loginFormErrors = {}
        for (const field in loginData) {
            clearLoginError(field)
        }

        // Retorna botão ao estado normal
